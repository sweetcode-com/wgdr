...
class WGDR {
	...
	public function init() {
		...
		add_filter( 'plugin_action_links_' . plugin_basename(__FILE__), array( $this, 'wgdr_settings_link' ) );
	...
	public function run_retargeting_for_visitor() {
		...
		if ( ! current_user_can( 'edit_others_pages' ) ) {
		    ...
		    if ( ! class_exists('wgact') && ( $this->gtag_deactivation == 0 ) ){
		        add_action( 'wp_head', array( $this, 'google_gtag' ) );
            }
	...
	function wgdr_plugin_admin_init() {
		...
		register_setting( 'wgdr_plugin_options_settings_fields', 'wgdr_plugin_options', array( $this, 'wgdr_plugin_options_validate')  );
	...
	public function wgdr_plugin_option_gtag_deactivation() {
		$options = get_option( 'wgdr_plugin_options' );
		?>
        <input type='checkbox' id='wgdr_plugin_option_gtag_deactivation' name='wgdr_plugin_options[gtag_deactivation]' value='1' <?php checked( $options['gtag_deactivation'] ); ?> />
        <?php
	...
	public function google_dynamic_retargeting_code() {
		...
		<script type="text/javascript">
			gtag(\'event\', \'page_view\', {
				...
				\'ecomm_totalvaluee\': ' . $product->get_price() . '
			});
		</script>';
