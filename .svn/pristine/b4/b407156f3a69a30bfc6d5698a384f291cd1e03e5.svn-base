...
class WGDR {
	...
	function wgdr_options_init() {
		...
		if ( false === $wgdr_options ) {
			$wgdr_options = $this->wgdr_get_default_options();
		}
	...
	*/
	...
	function wgdr_plugin_setting_string_2() {
		...
		echo "<input id='wgdr_plugin_text_string_2' name='wgdr_plugin_options_2[text_string]' size='40' type='text' value='{$options['text_string']}' /><br>This field is optional. Leave it empty if you don't use a customized AdWords retargeting tag.";
	}
