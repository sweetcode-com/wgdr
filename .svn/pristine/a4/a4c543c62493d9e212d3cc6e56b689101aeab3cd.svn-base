...
class DB_Upgrade{
	...
	public function run_options_db_upgrade(){
		...
		if( ! empty( get_option( 'wgdr_plugin_options_1' ) ) or ! empty( get_option( 'wgdr_plugin_options_2' ) ) or ! empty( get_option( 'wgdr_plugin_options_3' ) ) ){
	...
	public function upgrade_options_db_from_zero_to_1_point_zero(){
		...
		// get option 1
		if ( empty( get_option( $option_name_1 ) ) ) {
			$option_value_1 = "";
		} else {
		...
		// get option 3
		if ( empty( get_option( $option_name_3 ) ) ) {
			$option_value_3 = "";
		} else {
