...
class DB_Upgrade {
	...
	public function run_options_db_upgrade() {
		...
		// upgrade the options with the default identifier if it is empty
	...
	public function set_default_product_identifier_if_empty() {
		...
		get_option( $this->options_db_name );

		// find out if the option is empty
		if ( ! isset( $options_db_name['product_identifier'] ) ) {

			// set the option to its default value
			$options_db_name['product_identifier'] = 0;
		}
	}
}