	...
	**/
	...
	public function google_dynamic_retargeting_code(){
		...
		elseif ( is_order_received_page() ){
			...
			// And use the order met to check if the conversion code has already run for this order ID. If yes, don't run it again.
			// WC()->session->set_customer_session_cookie(true);
			// if ( !$order->has_status( 'failed' ) && ((WC()->session->get( 'single_conversion_identifier' ) != $order_key))){
			// if ( !$order->has_status( 'failed' ) && ((get_transient( 'wgdr_transient_ID_' . $order_key ) != $order_key))){
	...
	public function get_product_category( $product_id ){
		...
		// replace all ' with \' to produce a valid javascript parameter
