	...
	**/
	...
	public function google_dynamic_retargeting_code(){
		...
		elseif (is_order_received_page()){
			...
			// And use the WooCommerce session to check if the conversion code has already run for this order ID. If yes, don't run it again. 
		...
		<?php
				// Set the order ID session after the conversion code has run once.
				// WC()->session->set( 'single_conversion_identifier' , $order_key );
				set_transient( 'wgdr_transient_ID_' . $order_key, $order_key, 60*60*12 );
			} // end if order status
		}
